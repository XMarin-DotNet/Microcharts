...
namespace Microcharts
{
	...
	public abstract class Chart
	{
		...
		IDictionary<Rectangle, Action> _touchHandle = new Dictionary<Rectangle, Action>();

		public void AddTouchHandler(Rectangle region, Action handler)
		{
			if (!_touchHandle.ContainsKey(region))
				_touchHandle.Add(region, handler);
		}

		public bool HandleTouch(float x, float y)
		{
			var handled = false;

			foreach (var handle in _touchHandle)
				if (handle.Key.Contains(x, y))
				{
					handle.Value();
					handled = true;
				}
